...
#if defined(CONFIG_HW_ANDORRA) || defined(CONFIG_HW_ANDORRAP)
...
#endif
#define GPIO_SKUD_LCD_BRDG_RESET_N 129
#define GPIO_SKUD_LCD_BACKLIGHT_EN 28
#define GPIO_SKUD_LCD_ID 11

#define GPIO_LENOVO_LCD_BRDG_RESET_N 129
#define GPIO_LENOVO_LCD_BACKLIGHT_EN 28
#define GPIO_LENOVO_LCD_ID 11
#define LENOVO_CHECK_LCD_CONNECT_STATE

#ifdef  LENOVO_CHECK_LCD_CONNECT_STATE
#define LCD_EXIST	1
#define LCD_NOT_EXIST 0
static int lcd_connect_state = LCD_NOT_EXIST;
#endif
int g_lcd_id_high = 0;
...
static int __init boe_lcd(char *p)
{
	g_lcd = simple_strtoul(p, NULL, 0);
	printk("@@@@@@@@@@@@@@@@@@@@@@@@@@g_lcd = %d@@@@@@@@@@@@@@@@@@@@@@@@@@\n",g_lcd);
	return 0;
}
early_param("boelcd", boe_lcd);
#endif
...
#ifdef LENOVO_CHECK_LCD_CONNECT_STATE
static int lenovo_lcd_is_exist(void)
{
	static int lcd_gpio_detected = 0;
	int rc = 0;

	if (!lcd_gpio_detected) {
		rc = gpio_request(GPIO_LENOVO_LCD_ID, "lenovo_lcd_connection_state");
		if (rc < 0) {
			pr_err("%s: gpio_request lenovo_lcd_connection_state failed!",
					__func__);
		}
...
static int lenovo_panel_is_himax(void)
{
	static int lcd_gpio_detected = 0;
	...
	if (!lcd_gpio_detected) {
		rc = gpio_request(GPIO_LENOVO_LCD_ID, "lenovo_lcd_detect");
		if (rc < 0) {
			pr_err("%s: gpio_request lenovo_lcd_detect failed!",
					__func__);
			goto done;
		}
		...
		rc = gpio_tlmm_config(GPIO_CFG(GPIO_LENOVO_LCD_ID, 0,
					...
					GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		if (rc < 0) {
			pr_err("%s: unable to config lenovo_lcd_detect\n", __func__);
			gpio_free(GPIO_LENOVO_LCD_ID);
			goto done;
		}

		is_himax = gpio_get_value(GPIO_LENOVO_LCD_ID);
		printk("#####%s: LCD id pin = %d#########\n", __func__, is_himax);
  ...
  static int msm_lenovo_lcd_check(const char *name)
{
	int ret = -ENODEV;
	printk("####%s: name=%s###%d#\n", __func__, name,g_lcd_id_high);
	if(g_lcd_id_high)
	{
#if defined(CONFIG_HW_ANDORRA) || defined(CONFIG_HW_ANDORRAP)
		if (!strncmp(name, "mipi_cmd_yassy_qhd", 18))
			ret = 0;
#elif defined( CONFIG_HW_ATHENAE)
		if (!strncmp(name, "mipi_cmd_boe_fwvga", 18))
			ret = 0;
#else
		if (!strncmp(name, "mipi_cmd_himax_wvga", 19))
			ret = 0;
#endif
	}else
  	{
#if defined(CONFIG_HW_ANDORRA) || defined(CONFIG_HW_ANDORRAP)
		if (!strncmp(name, "mipi_cmd_nt35516_qhd", 20))
			ret = 0;
#elif defined(CONFIG_HW_ATHENAE)
		if (!strncmp(name, "mipi_cmd_boe_fwvga", 18))
			ret = 0;
#else
		if (!strncmp(name, "mipi_cmd_GL4502F_wvga", 23))
			ret = 0;
#endif
	}
	return ret;
...
{
	int ret = -ENODEV;
	ret = msm_lenovo_lcd_check(name);
	return ret;
	
	if (machine_is_msm7x27a_surf() || machine_is_msm7625a_surf() ||
			machine_is_msm8625_surf()) {
		printk("#surf###%s: name=%s####\n", __func__, name);
...
static struct msm_panel_common_pdata mipi_yassy_pdata = {
	...
	.gpio = GPIO_LENOVO_LCD_BACKLIGHT_EN,
};

static struct platform_device mipi_dsi_yassy_panel_device = {
	.name = "mipi_yassy",
	.id   = 0, 
	.dev  = {  
		.platform_data = &mipi_yassy_pdata,
	}    
};
...
static struct platform_device mipi_dsi_otm8009a_panel_device = {
	...
	.id   = 0, 
	.dev  = {  
		.platform_data = &mipi_otm8009a_pdata,
	}    
};

#else
static struct msm_panel_common_pdata mipi_GL4502F_pdata = {
	.backlight_level = NULL, //set in tps61161 driver
	.gpio = GPIO_LENOVO_LCD_BACKLIGHT_EN,
};

static struct platform_device mipi_dsi_GL4502F_panel_device = {
	.name = "mipi_GL4502F_alaska",
	.id   = 0,
	.dev  = {
		.platform_data = &mipi_GL4502F_pdata,
	}
};
...
#endif
static struct msm_panel_common_pdata mipi_general_pdata = {
	.backlight_level = NULL, //set in tps61161 driver
	.gpio = GPIO_LENOVO_LCD_BACKLIGHT_EN,
};

static struct platform_device lenovo_backlight_device = {
#if defined(CONFIG_HW_ANDORRA) || defined(CONFIG_HW_ANDORRAP)
	...
	.id   = 0,
	.dev  = {
		.platform_data = &mipi_general_pdata, //Himax or NT35510
	}
};
...
static struct platform_device *skud_fb_devices[] __initdata = {
	&msm_fb_device,
	&lenovo_backlight_device,
};

static struct platform_device *lenovo_fb_devices[] __initdata = {
	&msm_fb_device,
#if defined(CONFIG_HW_ANDORRA) || defined(CONFIG_HW_ANDORRAP)
	&mipi_dsi_NT35516_panel_device,//qhd
#elif defined(CONFIG_HW_ATHENAE)
	&mipi_dsi_otm8009a_panel_device,//fwvga
#else
	&mipi_dsi_GL4502F_panel_device, //NT35510 or Himax
#endif
	&lenovo_backlight_device,
};
...
#define GPIO_QRD3_LCD_BRDG_RESET_N	129//85
#define GPIO_QRD3_LCD_BACKLIGHT_EN	28//96
...
static unsigned skud_mipi_dsi_gpio[] = {
	...
	GPIO_CFG(GPIO_SKUD_LCD_BACKLIGHT_EN, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_UP,
			GPIO_CFG_2MA), /* GPIO_QRD3_LCD_BACKLIGHT_EN */
};
...
static int msm_fb_dsi_client_skud_reset(void)
{
	...
	rc = gpio_request(GPIO_SKUD_LCD_BACKLIGHT_EN, "skud_gpio_bkl_en");
	if (rc < 0) {
		pr_err("Failed to request skud_gpio_bkl_en\n");
		return rc;
	}
#endif
	return rc;
...
static unsigned lenovo_mipi_dsi_gpio[] = {
	GPIO_CFG(GPIO_LENOVO_LCD_BRDG_RESET_N, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_NO_PULL,
			GPIO_CFG_2MA), /* GPIO_LENOVO_LCD_BRDG_RESET_N */
	GPIO_CFG(GPIO_LENOVO_LCD_BACKLIGHT_EN, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_UP,
			GPIO_CFG_2MA), /* GPIO_QRD3_LCD_BACKLIGHT_EN */
};

static int msm_fb_dsi_client_reset_lenovo(void)
{
	int rc = 0;
	pr_err("####%s###########\n", __func__);
	
	rc = gpio_request(GPIO_LENOVO_LCD_BRDG_RESET_N, "lenovo_lcd_brdg_reset_n");
	if (rc < 0) {
		pr_err("failed to request lenovo lcd brdg reset_n\n");
		return rc;
	}
	//gpio_free(GPIO_LENOVO_LCD_BACKLIGHT_EN);
	//rc = gpio_request(GPIO_LENOVO_LCD_BACKLIGHT_EN, "lenovo_gpio_bkl_en");
	//if (rc < 0) {
	//	pr_err("Failed to request lenovo_gpio_bkl_en\n");
		//return rc;
	//}

		/*Configure LCD Bridge reset*/
		//rc = gpio_tlmm_config(lenovo_mipi_dsi_gpio[0], GPIO_CFG_ENABLE);
		//if (rc < 0) {
...
static int msm_fb_dsi_client_reset(void)
{
	...
	rc = msm_fb_dsi_client_reset_lenovo();
	return rc;
...
static int lenovo_dsi_gpio_initialized;
int mipi_dsi_panel_power_lenovo(int on)
{
	int rc = 0;
	pr_err("####%s###########%d\n", __func__, on);

	if (!lenovo_dsi_gpio_initialized) {
#ifdef CONFIG_HW_ARMANI
		pmapp_disp_backlight_init();
#endif
#ifdef CONFIG_HW_AUDI
                pmapp_disp_backlight_init();
#endif

		lenovo_dsi_gpio_initialized = 1;

		if (1)//mdp_pdata.cont_splash_enabled) 
		{
			/*Configure LCD Bridge reset*/
			rc = gpio_tlmm_config(lenovo_mipi_dsi_gpio[0],
			     GPIO_CFG_ENABLE);
			if (rc < 0) {
				pr_err("Failed to enable LCD Bridge reset enable\n");
				return rc;
			...
			rc = gpio_direction_output(GPIO_LENOVO_LCD_BRDG_RESET_N, 1);

			if (rc < 0) {
				pr_err("Failed GPIO bridge Reset\n");
				gpio_free(GPIO_LENOVO_LCD_BRDG_RESET_N);
				return rc;
			}

			rc = gpio_tlmm_config(lenovo_mipi_dsi_gpio[1], GPIO_CFG_ENABLE);
			if (rc < 0) {
				pr_err("Failed to config lenovo_gpio_bkl_en\n");
				gpio_free(GPIO_LENOVO_LCD_BACKLIGHT_EN);
				return rc;
			}
			//rc = gpio_direction_output(GPIO_LENOVO_LCD_BACKLIGHT_EN, 1);

#ifdef LENOVO_CHECK_LCD_CONNECT_STATE
			if (lcd_connect_state == LCD_NOT_EXIST)
#endif
#if defined(CONFIG_HW_ARMANI) || defined(CONFIG_HW_AUDI)

#ifdef LENOVO_CHECK_LCD_CONNECT_STATE 
			{
				printk("****** lcd no connection, close backlight *******\n");
				pmapp_disp_backlight_set_brightness(0);
			 } else
#endif
				pmapp_disp_backlight_set_brightness(100);
#else
#ifdef LENOVO_CHECK_LCD_CONNECT_STATE
				pmic_gpio_set_value(0, 0);
#endif
#endif
#if defined(CONFIG_HW_ARMANI) || defined(CONFIG_HW_AUDI) || defined(CONFIG_HW_ANDORRA) || defined(CONFIG_HW_ANDORRAP)
			return 0;
#endif
		}
	}
	...
	if (on) {
		printk("**************** lcd reset ******************\n");
		gpio_set_value_cansleep(GPIO_LENOVO_LCD_BRDG_RESET_N, 1);
		msleep(30);
		gpio_set_value_cansleep(GPIO_LENOVO_LCD_BRDG_RESET_N, 0);
		msleep(30);
		gpio_set_value_cansleep(GPIO_LENOVO_LCD_BRDG_RESET_N, 1);
		msleep(20);
	} else {
...
static int mipi_dsi_panel_skud_power(int on)
{
	...
	if (!skud_dsi_gpio_initialized) {
		...
		if (mdp_pdata.cont_splash_enabled) {
			...
			rc = gpio_tlmm_config(skud_mipi_dsi_gpio[1], GPIO_CFG_ENABLE);
			if (rc < 0) {
				pr_err("Failed to config skud_gpio_bkl_en\n");
				gpio_free(GPIO_SKUD_LCD_BACKLIGHT_EN);
				return rc;
			}
			rc = gpio_direction_output(GPIO_SKUD_LCD_BACKLIGHT_EN, 1);
			return 0;
		}
	}
...
static int mipi_dsi_panel_power(int on)
{
	...
	rc = mipi_dsi_panel_power_lenovo(on);
	return rc;
...
static int __init prim_display_setup(char *param)
{
	if (strnlen(param, PANEL_NAME_MAX_LEN))
	{
		...
		pr_err("#######prim_display_setup msm_fb_pdata.prim_panel_name %s#########\n", msm_fb_pdata.prim_panel_name);
	}
	return 0;
}
...
void msm7x27a_set_display_params(char *prim_panel)
{
	...
	pr_err("#####msm_fb_pdata.prim_panel_name %s#######%d###\n", msm_fb_pdata.prim_panel_name, disable_splash);
}

static struct i2c_board_info lm3528_device_info[] __initdata = {
	{
...
void __init msm_fb_add_devices(void)
{
	int rc = 0;
	/* default is NT35510 */
	g_lcd_id_high = lenovo_panel_is_himax();

	i2c_register_board_info(MSM_GSBI1_QUP_I2C_BUS_ID,	lm3528_device_info,	ARRAY_SIZE(lm3528_device_info));
	...
	if (g_lcd_id_high) {
#if defined(CONFIG_HW_ANDORRA) || defined(CONFIG_HW_ANDORRAP)
		lenovo_fb_devices[1] = &mipi_dsi_yassy_panel_device;
		lenovo_backlight_device.dev.platform_data = &mipi_yassy_pdata;
#elif defined(CONFIG_HW_ATHENAE)
		lenovo_fb_devices[1] = &mipi_dsi_otm8009a_panel_device;
		lenovo_backlight_device.dev.platform_data = &mipi_otm8009a_pdata;
#else
		lenovo_fb_devices[1] = &mipi_dsi_himax_panel_device;
		lenovo_backlight_device.dev.platform_data = &mipi_himax_pdata;
#endif
	} else {
#if defined(CONFIG_HW_ANDORRA) || defined(CONFIG_HW_ANDORRAP)
		lenovo_backlight_device.dev.platform_data = &mipi_NT35516_pdata;
#elif defined(CONFIG_HW_ATHENAE)
		lenovo_backlight_device.dev.platform_data = &mipi_otm8009a_pdata;
#else
		lenovo_backlight_device.dev.platform_data = &mipi_GL4502F_pdata;
#endif
	}
	...
	if(lenovo_lcd_is_exist() == LCD_NOT_EXIST) {
		...
		lenovo_backlight_device.name = "xxxx";
	}
#endif
	platform_add_devices(lenovo_fb_devices,
			ARRAY_SIZE(lenovo_fb_devices));
