...
/* yangjq, 20121127, Add for vreg debug, START */
/* sub commands of PCOM_VREG_CTRL */
#define VREG_CTRL_GET_STATUS 1
#define VREG_CTRL_SET        2
#define VREG_CTRL_GET_LAST_SLEEP_STATUS 3

#define MV_INVALID  (0xffff)  // invalid voltage

extern int vreg_get_status(int subcommand, unsigned id, unsigned* on, unsigned* mv);
static int _vreg_dump_info(int subcommand, char* buf)
{
	char* p = buf;
	struct regulator_map *map;
	struct regulator_dev *rdev;
	unsigned on, mv;
	int id;
	u16 vreg_status_total;
	u16 vreg_status_diff;
	int fetched;

	fetched = 0;
	vreg_status_total = vreg_status_diff = 0;
	if(subcommand == VREG_CTRL_GET_LAST_SLEEP_STATUS) {
		p += sprintf(p, "vreg_last_sleep:\n");
		if(vreg_get_status(subcommand, 256, &on, &mv)) {
			fetched = on & 0x0000ffff;
			vreg_status_total = on >> 16;
			vreg_status_diff = mv >> 16;
		}
		if (fetched) {
			return p - buf;
		}
		if(vreg_status_total == 0 && vreg_status_diff == 0) {
			p += sprintf(p, "diff/total=%d/%d\n", vreg_status_diff, vreg_status_total);
			return p - buf;
		}
	}
	...
	list_for_each_entry(map, &regulator_map_list, list) {
		...
		id = rdev->desc->id;
		p += sprintf(p, "[%2d]%10s/%-10s: ", id, rdev->desc->name, map->supply);

		if(vreg_get_status(subcommand, rdev->desc->id, &on, &mv)) {
			if(subcommand == VREG_CTRL_GET_LAST_SLEEP_STATUS) {
				vreg_status_total = on >> 16;
				if (vreg_status_total > 0) {
					vreg_status_diff = mv >> 16;
					on &= 0x0000ffff;
					mv &= 0x0000ffff;
				}
...
{
	return _vreg_dump_info(VREG_CTRL_GET_STATUS, buf);
}

int vreg_last_sleep_dump_info(char* buf)
{
	return _vreg_dump_info(VREG_CTRL_GET_LAST_SLEEP_STATUS, buf);
}

extern void vreg_config(struct regulator_dev *rdev, unsigned on, unsigned mv);
void regulator_config(struct regulator *reg, unsigned on, unsigned mv)
{
	vreg_config(reg->rdev, on, mv);
}
/* yangjq, 20121127, Add for vreg debug, END */
