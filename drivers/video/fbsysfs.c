			   ...
			   const char *buf, size_t count)
{
	char *last = NULL;
    int key_blan;
	key_blan = simple_strtoul(buf, &last, 0);
	printk("store_keyblan key_blan = %d\n",key_blan);
	console_lock();
    msm_fb_set_keyboardbacklight(key_blan);
	console_unlock();
	return count;
}

static ssize_t show_keyblan(struct device *device,
			  struct device_attribute *attr, char *buf)
{
//	struct fb_info *fb_info = dev_get_drvdata(device);
	return 0;
